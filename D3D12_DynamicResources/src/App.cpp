...
#include "../res/shader/Compiled/SimpleVS.inc"
...
bool App::InitD3D()
{
    ...
    if ( FAILED( hr ) )
    {
        ELOG( "Error : CreateDXGIFactory() Failed. errcode = 0x%x", hr );
        return false;
    }
    ...
    // ID3D12InfoQueueに変換.
    {
        asdx::RefPtr<ID3D12InfoQueue> pInfoQueue;

        hr = m_pDevice->QueryInterface(IID_PPV_ARGS(pInfoQueue.GetAddress()));
        if (SUCCEEDED(hr))
        {
            // エラー発生時にブレークさせる.
            ...
            pInfoQueue->SetBreakOnID(D3D12_MESSAGE_ID_CLEARRENDERTARGETVIEW_MISMATCHINGCLEARVALUE, FALSE);
            pInfoQueue->SetBreakOnID(D3D12_MESSAGE_ID_CLEARDEPTHSTENCILVIEW_MISMATCHINGCLEARVALUE, FALSE);
        }
    }
#endif

    // Resource Binding Tier 3のサポートチェック.
    D3D12_FEATURE_DATA_D3D12_OPTIONS data;
    hr = m_pDevice->CheckFeatureSupport(D3D12_FEATURE_D3D12_OPTIONS, &data, sizeof(data));
    if (FAILED(hr))
    {
        ELOG("Error : ID3D12Device::CheckFeatureSupport() Failed. errcode = 0x%x", hr);
        return false;
    }

    if (data.ResourceBindingTier != D3D12_RESOURCE_BINDING_TIER_3) 
    {
        ELOG("Error : Not Support D3D12_RESOURCE_BINDING_TIER3");
        return false;
    }

    // Shader Model 6.6のサポートチェック.
    ...
    {
        ELOG("Error : Not Support Shader Model 6.6");
        return false;
    }
...
bool App::InitApp()
{
    ...
    // パイプラインステートの生成.
    {
        ...
        desc.VS                                 = { g_VSFunc, sizeof(g_VSFunc) };
        desc.PS                                 = { g_PSFunc, sizeof(g_PSFunc) };
