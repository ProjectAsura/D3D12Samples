...
bool App::InitD3D()
{
    ...
    //if ( FAILED( hr ) )
    if (!CreateGraphicsCommandList(m_pDevice.GetPtr(), D3D12_COMMAND_LIST_TYPE_DIRECT, m_pCmdAllocator.GetPtr(), m_pCmdList.GetAddress()))
    {
...
//    ID3D12CommandList* ppCmdLists[] = { m_pCmdList.GetPtr() };
    ID3D12CommandList* ppCmdLists[] = { m_pCmdList->GetCommandList() };
